...
def nbd_open_channel(uri):
    ...
    sock.sendall(
        "PUT /services/SM/nbd/%s/%s/%s?session_id=OpaqueRef%%3a%s HTTP/1.1\r\nHost: %s\r\n\r\n" % (sr_uuid, vdi_uuid, dp_uuid, session_id, server))
...
def nbd_send_write(sock, handle, request_handles, offset, length, data):
    ...
    while length > 0:
        ...
        if length >= NBD_CHUNK_SIZE:
            ...
            DEBUG("NBD: buffer_offset = %d, offset = %d, length = %d, calculated length of data to send = %d,length of _buffer_ %d" % (
            buffer_offset, offset, length, NBD_CHUNK_SIZE, len(_buffer_)))
        else:
            ...
            DEBUG("NBD: buffer_offset = %d, offset = %d, length = %d, calculated length of data to send = %d,length of _buffer_ %d" % (
            buffer_offset, offset, length, length, len(_buffer_)))
...
def rbd2vhd(rbd, vhd, rbd_image_uuid, progress, mrout):
    ...
    while True:
        ...
        if not record_tag:
            ...
            if (rbd_meta_read_finished == 1) & (vhd_headers_written == 0):
                ...
                if parent_exists:
                    vhd_dynamic_disk_header_struct = gen_vhd_dynamic_disk_header_struct(VHD_FOTTER_RECORD_SIZE + VHD_DYNAMIC_DISK_HEADER_RECORD_SIZE,
                                                                                        image_size, 0, parent_uuid.bytes, "%s.vhd" % str(parent_uuid))
                    vhd_dynamic_disk_header_struct = modTupleByIndex(vhd_dynamic_disk_header_struct, _dynamic_disk_header_parent_time_stamp_,
                                                                     time.time() - 946684800)  # ????
                else:
                    vhd_dynamic_disk_header_struct = gen_vhd_dynamic_disk_header_struct(VHD_FOTTER_RECORD_SIZE + VHD_DYNAMIC_DISK_HEADER_RECORD_SIZE,
                                                                                        image_size, 0, '', '')
                VHD_DYNAMIC_DISK_HEADER = pack(VHD_DYNAMIC_DISK_HEADER_FORMAT, *vhd_dynamic_disk_header_struct)
                vhd_dynamic_disk_header_struct = modTupleByIndex(vhd_dynamic_disk_header_struct, _dynamic_disk_header_checksum_,
                                                                 checksum(VHD_DYNAMIC_DISK_HEADER))
                VHD_DYNAMIC_DISK_HEADER = pack(VHD_DYNAMIC_DISK_HEADER_FORMAT, *vhd_dynamic_disk_header_struct)
                ...
                for index in range(VHD_PARENT_LOCATORS_COUNT):
                    if parent_exists:
                        if index == 0:
                            ...
                            parent_locator_entry_struct = (
                            _platform_code_MacX_, get_size_aligned_to_sector_boundary(len(parent_locator_encoded)), len(parent_locator_encoded), 0,
                            vhd_file_offset)  # MacX
                        elif index == 1:
                            parent_locator = ".\%s.vhd" % parent_uuid
                            parent_locator_encoded = parent_locator.encode("UTF-16LE")
                            parent_locator_entry_struct = (
                            _platform_code_W2ku_, get_size_aligned_to_sector_boundary(len(parent_locator_encoded)), len(parent_locator_encoded), 0,
                            vhd_file_offset)  # W2ku
                        elif index == 2:
                            parent_locator = ".\%s.vhd" % parent_uuid
                            parent_locator_encoded = parent_locator.encode("UTF-16LE")
                            parent_locator_entry_struct = (
                            _platform_code_W2ru_, get_size_aligned_to_sector_boundary(len(parent_locator_encoded)), len(parent_locator_encoded), 0,
                            vhd_file_offset)  # W2ru
                        else:
                    ...
                    if len(parent_locator_encoded) > 0:
                        parent_locator_packed = pack("!%ds" % get_size_aligned_to_sector_boundary(len(parent_locator_encoded)),
                                                     parent_locator_encoded)
                    else:
                    ...
                    vhd_dynamic_disk_header_struct = modTupleByIndex(vhd_dynamic_disk_header_struct,
                                                                     _dynamic_disk_header_parent_locator_entry_1_ + index, parent_locator_entry)
            ...
            if (rbd_meta_read_finished == 1) & (vhd_headers_written == 1) & (rbd_eof == False):
                ...
                while length > 0:
                    ...
                    if vhd_bat_list[BlockNumber] == 0xffffffff:
                        ...
                        DEBUG("VHD: Write %d bytes of empty sectors bitmap" % len(
                            gen_bitmap_from_bitarray(gen_empty_bitarray_for_bitmap(block_bitmap_size))))
    ...
    if (rbd_data_exists == True):
        ...
        VHD_FH.write(VHD_BAT)
        vhd_file_offset += len(VHD_BAT)
        INFO("VHD: Rewrite BAT (write %d entries, %d bytes)" % (vhd_dynamic_disk_header_struct[_dynamic_disk_header_max_table_entries_],
                                                                vhd_dynamic_disk_header_struct[_dynamic_disk_header_max_table_entries_] * 4))

        DEBUG("VHD: Current offset in VHD file is 0x%08x" % vhd_file_offset)

        for BlockNumber in range(len(vhd_bat_list)):
            if vhd_bat_list[BlockNumber] != 0xffffffff:
                DEBUG("VHD: Block %d offset is 0x%08x, skeep 0x%08x bytes from last offest 0x%08x" % (
                BlockNumber, vhd_bat_list[BlockNumber] * SECTOR_SIZE, (vhd_bat_list[BlockNumber] * SECTOR_SIZE - vhd_file_offset), vhd_file_offset))
...
def vhd2rbd(vhd, rbd, progress, mrout):
    ...
    if DYNAMIC_DISK_HEADER[_dynamic_disk_header_parent_unique_id_] != '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00':
        ...
        RBDDIFF_FH.write(
            pack("%s%s%s%ds" % (RBD_DIFF_META_ENDIAN_PREFIX, RBD_DIFF_META_RECORD_TAG, RBD_DIFF_META_SNAP, len(from_snap)), 'f', len(from_snap),
                 from_snap))
    ...
    if VHD_FOOTER[_vhd_footter_unique_iq_] != VHD_FOOTER[_vhd_footter_rbd_image_uuid_]:
        ...
        RBDDIFF_FH.write(
            pack("%s%s%s%ds" % (RBD_DIFF_META_ENDIAN_PREFIX, RBD_DIFF_META_RECORD_TAG, RBD_DIFF_META_SNAP, len(to_snap)), 't', len(to_snap), to_snap))
    ...
    for block_index in range(DYNAMIC_DISK_HEADER[_dynamic_disk_header_max_table_entries_]):
        if BAT_TABLE[block_index] != 0xffffffff:
            ...
            for sector_in_block_index in range(BITMAP_SIZE * 8):
                if BITMAPARRAY[sector_in_block_index] == 1:
                    ...
                    if raw_first_sector == -1:
                        raw_first_sector = get_raw_sector_offset_of_sector(block_index, sector_in_block_index,
                                                                           DYNAMIC_DISK_HEADER[_dynamic_disk_header_block_size_], SECTOR_SIZE)
                        in_block_first_sector = sector_in_block_index
                        raw_last_sector = -1
                else:
                    if (raw_last_sector == -1) & (raw_first_sector != -1):
                        raw_last_sector = get_raw_sector_offset_of_sector(block_index, sector_in_block_index - 1,
                                                                          DYNAMIC_DISK_HEADER[_dynamic_disk_header_block_size_], SECTOR_SIZE)
                        ...
                        RBDDIFF_FH.write(pack(RBD_DIFF_META_ENDIAN_PREFIX + RBD_DIFF_META_RECORD_TAG + RBD_DIFF_DATA, 'w', raw_first_sector * 512,
                                              (raw_last_sector - raw_first_sector + 1) * 512))
            ...
            if (sector_in_block_index == BITMAP_SIZE * 8 - 1) and (raw_first_sector != -1):
                raw_last_sector = get_raw_sector_offset_of_sector(block_index, sector_in_block_index,
                                                                  DYNAMIC_DISK_HEADER[_dynamic_disk_header_block_size_], SECTOR_SIZE)
